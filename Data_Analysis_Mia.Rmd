...
#gmdata <- slice(gmdata, -775:-868) #check columns each time
```
...
```{r}
F2 <- ggplot(gmdata)+
geom_histogram(aes(x = flav_Conc), binwidth = 0.07)+
  theme_bw()+
...
#############################WTF IS GOING ON WITH THE CLUSTER OF LOW VALUES############################################

#gmdata$gluc_Conc[grep(, gmdata$gluc_Conc)]

```
graph combos in bivariate plot and incl se and look for overlap
```{r}
gmdata$Tag[grep("JARI", gmdata$Tag)] #do this for all pop tags!
...
#exclude negative values for now for gamma glm #####since c1v1 = c2v2 fixed this problem, skip this
#gmdata$gluc_Conc[which(gmdata$gluc_Conc < 0)] <- NA
#gmdata$flav_Conc[which(gmdata$flav_Conc < 0)] <- NA
...
widegm <- widegm %>% mutate(meancba = mean(a, na.rm = TRUE), 
                            meancbgm = mean(gm, na.rm = T),
...
p1 <- ggplot(data = widegm, aes(x = meancba, y = meancbgm))+
  ...
  geom_errorbar(aes(ymin = meancbgm - secbgm, 
                ...
                width = .001, colour = "red"))+
  geom_errorbarh(aes(xmin = meancba - secba,
                    xmax = meancba + secba,
                    width = .001, colour = "blue"))+
  labs(y = "Chlorophyll b Intraspecific treatment", x = "Chlorophyll b Alone")+
  theme(legend.position = "none")
    
  
  #gm intra vs m inter
p2 <- ggplot(data = widegm, aes(x = meancbm, y = meancbgm))+
  geom_point()+
  geom_errorbar(aes(ymin = meancbgm - secbgm, 
                    ymax = meancbgm + secbgm, 
                width = .001, colour = "red"))+
  geom_errorbarh(aes(xmin = meancbm - secbm,
                    xmax = meancbm + secbm,
                    width = .001, colour = "blue"))+
  labs(y = "Chlorophyll b Intraspecific treatment", x = "Chlorophyll b Interspecific treatment")+
  theme(legend.position = "none")

  
  #alone vs inter m
p3 <- ggplot(data = widegm, aes(x = meancbm, y = meancba))+
  geom_point()+
  geom_errorbar(aes(ymin = meancba - secba, 
                    ymax = meancba + secba,
                width = .001, colour ="red"))+
  geom_errorbarh(aes(xmin = meancbm - secbm,
                    xmax = meancbm + secbm,
                    width = .001, colour = "blue"))+
  labs(y = "Chlorophyll b Alone", x = "Chlorophyll b Interspecific treatment")+
  theme(legend.position = "none")
