...
```{r}
#Selection gradient of survival (with bolt size)

library(lavaan)

#Bolt size variation was eliminating too much of the useful variation in total fecundity. I am removing this from the analysis for this reason. 
MapleFormula_1_1 <- '
 Maple_TotalLeafArea_EndS ~ PC1Tot+ B1*PCCG_1S + MapleDamage + B2*PCCG_2S
...
ExportX<-function(plot,outfile){
  
my_vec_graph <- dml(ggobj = plot)

outputPPT<-read_pptx() 
layout_summary(outputPPT)
...
TLATiff<-ggplot(datPlot)+
  ...
  ylab(bquote(bold("Rosette Size Residuals")))+xlab("General Glucosinolate and Chlorophyll investment (σ)")+
...
#Formating data frame for graphing
#---------
...
PathPerf<-ggraph(lay)  +
 ...
 geom_edge_link(
   ...
   aes(start_cap = label_rect(node1.name,padding =margin(3,3,3,3,"mm") ), end_cap = label_rect(node2.name,padding =margin(5,3,3,3,"mm")), width=abs(est),color=PosNeg,label=estL),
   fontface="bold",
   show.legend = F,
  label_colour = 'black',
  fill='white',
 ...
 # scale_edge_width(range=c(0,max(abs(b$est))))


PathPerf
c
#Graphing plot c
PathPerfC<-ggraph(lay2,circular=T)  +

 geom_edge_link(
   aes(start_cap = label_rect(node1.name,padding =margin(3,3,3,3,"mm") ), end_cap = label_rect(node2.name,padding =margin(5,3,3,3,"mm")),color=PosNeg,label=estL,width=as.factor(estL)),
   #width=6,
   fontface="bold",
   show.legend = F,
  label_colour = 'black',
  ...
  scale_x_continuous(limits=c(8,10))+
      scale_edge_colour_manual(values=c("#CC6666","#66CC99"))+
  scale_edge_width_manual(values=c("0.02 **"=6,"-0.01 **"=3))
...
ExportX(PathPerf,"Selection_Figures/PathPerf.pptx")
...
#### Total fitness Path analysis Visualization 
```{r}
library(ggraph)
library(igraph)
...
#Altering the p values in the model to reflect permutation tests.
 b$pvalue[b$from=="Maple_TotalLeafArea_EndS"]<-0.2805
  b$pvalue[b$from=="PCCG_1S"& b$to=="GM_Fecundity2MS"]<-0.0838
 b$pvalue[b$from=="PCCG_2S"& b$to=="GM_Fecundity2MS"]<-0.0487
 b$pvalue[b$from=="indirect_PCCG_2"]<- 0.1521
  b$pvalue[b$from=="indirect_PCCG_1"]<-0.1782
#Adding significance to b values. 
 b$estL<-b$est
  b$estL[b$pvalue>=0.05]<- round(b$est[b$pvalue>=0.05],2)
 b$estL[b$pvalue<0.05]<-paste(round(b$est[b$pvalue<0.05],2),"*")
 ...
 b$estL[b$from=="PCCG_2S"& b$to=="GM_Fecundity2MS"]<-"-0.50*"
...
tiff("Selection_Figures/PathAnalysisTotalFit.tiff", units="in", width=8, height=4, res=300)
PathSurv
dev.off()
