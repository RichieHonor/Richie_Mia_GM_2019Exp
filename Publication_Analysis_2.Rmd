...
# 
# dat %>% 
#   filter(!grepl("maple",Tag,ignore.case = T),!is.na(gluc_Conc)) %>% 
#   group_by(Tag) %>% 
#   summarize(g=n()) %>% 
#   ungroup() %>% 
#   group_by(g) %>% 
...
dat2 <-
  ...
  summarize(
    ChlorA = mean(ChlorA),
    ChlorB = mean(ChlorB),
    gluc_Conc = mean(gluc_Conc),
    flav_Conc = mean(flav_Conc),
    Family = first(Family),
    treatment = first(treatment),
    gh_row = first(gh_row),
    gh_bench = first(gh_bench),
    GM_TotalLeaf_Area = first(GM_TotalLeaf_Area),
    comp_number = first(comp_number),
    ThripsDam = mean(ThripsDam),
    WhiteFungDam = mean(WhiteFungDam),
    BlackPathDam = mean(BlackPathDam),
    Fern = mean(Fern),
    gh_col = first(gh_col),
    GM_Leaf_Area = mean(GM_Leaf_Area),
...
#Finding sample size of leaf samples
dat2 %>% filter(!is.na(gluc_Conc)&!grepl("maple",Tag,ignore.case = T),!is.na(GM_Leaf_Area),!is.na(ChlorA)) %>% 
  group_by(LeafSamples) %>% 
  summarize(n())
```
...
#Number of samples may still be 1 even if no samples were obtained because dead ones were included. 

datSample <- dat2 %>% filter(treatment!="mcnt")

#Plants with no samples
datSample %>% filter(is.na(gluc_Conc))
#141 with no samples. 

#Plants with 1 sample
datSample %>% filter(!is.na(gluc_Conc),NumberOfSamples==1)
#98 plants with 1 sample

#Plants with two samples
datSample %>% filter(!is.na(gluc_Conc),NumberOfSamples==2)
#267 plants with 2 samples (one actually had three as described below, but this is added in)

#weird plant with 3 samples
datSample %>% filter(!is.na(gluc_Conc),!NumberOfSamples%in%c(1,2))


#All plants with a glucosinolate sample
datSample %>% filter(!is.na(gluc_Conc))
...
dat2 %>% filter(is.na(ChlorA),is.na(gluc_Conc),NumberOfSamples>0)

dat2 %>% filter(!is.na(gluc_Conc),NumberOfSamples==1)
...
GM_TotalLeaf_AreaMS ~ B3*Maple_TotalLeafArea_EndS2 + BlackPathDamS +FernS +PCCG_2S+ PCCG_1S + Transplant_Size_GMS
...
b$pvalue[b$from=="B1*B3"]<-0.0101 #indirect pc1
b$pvalue[b$from=="B2*B3"]<-0.0086 #indirect Pc2
b$pvalue[b$from=="Maple_TotalLeafArea_EndS"]<-0.0195
b$pvalue[b$from=="PCCG_2S"&b$to=="GM_TotalLeaf_AreaMS"]<- 0.1487 #direct Pc2
b$pvalue[b$from=="PCCG_1S"&b$to=="GM_TotalLeaf_AreaMS"]<- 0.0748 #direct Pc1
...
ExportX(PathPerf,PathPerfC,"Selection_Figures/PathPerfNew3.pptx")
