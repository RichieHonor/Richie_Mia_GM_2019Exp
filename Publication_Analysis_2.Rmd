...
#reassessing PCCG_1 when family is not included. 
fit<- glmmTMB(Bolt_Survival ~ treatment*Standardize(PCCG_1) + Standardize(Fern) +  
      (1 | Row_Field) ,data=dat2, family="binomial")
...
fit<- glmmTMB(Standardize(Bolt_Survival,meanSTD=T) ~ treatment + Standardize(Fern) +  
    Standardize(PCCG_2) + (1 | Row_Field) ,data=dat2)
...
#Attempting table creation with semi-automated selection. 
```{r}
...
SigExtract<-function(bestmodel,SigVar){
 #Extracting test statistic of every non significant variable: 
    
  #Model data with NAs of additional variable removed. 
    datFoc<-dat2 %>% drop_na(SigVar)
  ...
  #Removing any double + from removing variable.
  newFormula<-gsub("++","+",newFormula,fixed = T)
...
#Performance -- Table generation of test statistics
```{r}
#Best model 
fit6<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*PCCG_1+Fern+BlackPathDam+Transplant_Size_GM+(1|gh_bench),  data=dat2)
summary(fit6)

# Maximum model 
fitfull<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*PCCG_1+treatment*PCCG_2+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(Fern)+RGR1+Transplant_Size_GM+(1|Family)+(1|gh_bench), data=dat2)

SignificantVar<-c("Fern","BlackPathDam","Transplant_Size_GM")

NonSignificantVar<-c("PCCG_2","WhiteFungLogis","ThripsDam","RGR1")


#Model still does not work when there are interactions or random effects because it cannot remove NAs from those collumns. 

# Yay! Model extracting functions. 
...
for(i in 1:length(NonSignificantVar)){
  out<-NonSigExtract(fit6,NonSigVar=NonSignificantVar[i])
...
data.frame(Variable_Glob,Chi_Glob,p_Glob)
...
for(i in 1:length(SignificantVar)){
  out<-SigExtract(fit6,SigVar=SignificantVar[i])
...
#growth rate?
```{r}
#Calculates lambda based on the input of lifetime fitness for a popoulation
LamFun<-function(x){
  x<-x[!is.na(x)]
  x<-x*100
  nt1<-sum(x) #individuals at time 1, assuming 1 gram of silique results in 1 offspring.
  nt<-length(x) #Individuals at time 0
  return(nt1/nt)
}

#I am not sure if it makes sense to include treatment as there were no GXE interactions and if we are looking for the genetic correlation between a trait then we should just use the mean value. This analysis, however, doesnt control for nuissance variables impacting total fitness. Might be best to obtain these estimates from a mixed model. 

#Controlling for field row in lifetime fitness, the only signficant variable besides treamtent ----

#Removing Na's and making new dataframe with NAs excluded
datPlot<-dat2[!is.na(dat2$Row_Field)&!is.na(dat2$GM_Fecundity2),]

#For PCCG_1, Leaf Quality, obtaining the values of PCCG_1 after controlling for leaf area. 
fit<-glmmTMB(GM_Fecundity2~as.factor(Row_Field),data=datPlot)
summary(fit)
fitResi<-residuals(fit)
datPlot$GM_Fecundity2Controlled<-as.single(fitResi)


dat3.0<-dat2 %>% filter(treatment!="mcnt") %>% group_by(Family) %>% summarize(Lam=LamFun(GM_Fecundity2),PCCG_1=mean(PCCG_1,na.rm=T),PCCG_2=mean(PCCG_2,na.rm=T),Latitude=mean(Latitude,na.rm=T))


fit<-glmmTMB(Lam~PCCG_2,data=dat3.0,family=nbinom2)
fit<-glmmTMB(Lam~PCCG_2,data=dat3.0)

summary(fit)


fit<-glmmTMB(Lam~PCCG_1,data=dat3.0,family=nbinom2)
summary(fit)


fit3<-update(fit2,~.-PCCG_1)
anova(fit2,fit3)

fit4<-update(fit3,~.-PCCG_2:treatment)
anova(fit4,fit3)

fit5<-update(fit4,~.-PCCG_2)
anova(fit4,fit5)



#Getting summarized (pop mean) data. 

dat3<-dat2 %>% filter(treatment!="mcnt") %>% group_by(Family,treatment) %>% summarize(Lam=LamFun(GM_Fecundity2),PCCG_1=mean(PCCG_1,na.rm=T),PCCG_2=mean(PCCG_2,na.rm=T),Latitude=mean(Latitude,na.rm=T))

ggplot(dat3)+
  geom_point(aes(x=PCCG_1,y=Lam))

ggplot(dat3[dat3$treatment=="a",])+
  geom_point(aes(x=PCCG_2,y=Lam))

hist(dat3$Lam)

dat3[dat3$treatment=="m",]

fit<-glmmTMB(Lam~PCCG_1*treatment+PCCG_2*treatment,data=dat3,family=nbinom2)
fit2<-update(fit,~.-PCCG_1:treatment)
anova(fit,fit2)
...
#None significant but treatment. 
summary(fit5)

#Should control for the field row. 

hist(residuals(fit),breaks=20)
plot(residuals(fit)~fitted(fit))
plot(fitted(fit),fitted(fit)+residuals(fit))+abline(a=0,b=1)  
shapiro.test(residuals(fit)) 


fit<-glmmTMB(Lam~PCCG_2+treatment,data=dat3,family=nbinom2)
summary(fit)

plot(dat3.0$Latitude~dat3.0$PCCG_2)

summary(lm(dat3.0$Latitude~dat3.0$PCCG_2))
...
fix and redo permutation tests. 
```{r}
...
TLATiff<-ggplot(datPlot)+
  ...
  geom_segment(x=minG,xend=maxG,y=mustardSlope(minG),yend=mustardSlope(maxG),colour="#56B4E9",size=1.5)+theme_simple_2()+
