...
int<-"treatment:PCCG_2"
var<-"(1 | gh_bench)" #Must insert space between random effects .
model.frame(fit6)
...
#Best model 
fit6<-glmmTMB(Standardize(GM_TotalLeaf_Area)~PCCG_1+treatment:PCCG_1+Fern+BlackPathDam+Transplant_Size_GM+(1 | gh_bench),  data=dat2)
...
# Maximum model 
fitfull<-fit<-glmmTMB(log(GM_Fecundity)~treatment*PCCG_1+treatment*PCCG_2+Standardize(Fern)+BlackPathDam+Standardize(GM_TotalLeaf_Area)+Standardize(RGR1)+(1|Family)+(1|Col_Field)+Transplant_Size_GM,data=dat2)
...
SignificantVar<-c("treatment:PCCG_1"+"treatment" , "Fern" ,  
...
NonSignificantVar<-c("PCCG_2","WhiteFungLogis","RGR1", "treatment:PCCG_2","treatment:PCCG_1","PCCG_1","BlackPathDam","ThripsDam","WhiteFungLogis","(1 | Col_Field)","GM_TotalLeaf_AreaS", "Fern","Transplant_Size_GM", "(1 | gh_bench)")
...
output<-a %>% full_join(b,by="Variable_Glob") %>% full_join(c,by="Variable_Glob") %>% left_join(d,by="Variable_Glob")
