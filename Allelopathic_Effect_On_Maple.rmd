...
dat$Maple_TotalLeafArea_End<-Standardize(dat$Maple_TotalLeafArea_End)
hist(dat$ThripsDamW)
hist(dat$BlackPathDamW)
...
anova(fit3,fit2) #There is a significant two way interaction.

fit4<-update(fit2,~.-BlackPathDamW:ThripsDamW)
anova(fit4,fit2)
#There is a significant interaction between black path dam and thrips dam interaction p=0.08

fit5<-update(fit2,~.-WhiteFungLogis:ThripsDamW)
anova(fit5,fit2) #There is a significant white path and thripsdamage interaction.

summary(fit2)
#Fern and all pathogen damages influence fitness. Logging this data added alot of power to the analysis. This is because it removed the influence of very high values in influencing the model. 
...
fit7<-lmer(GM_TotalLeaf_Area ~ Maple_TotalLeafArea_End + log(Fern + 1) +  
    log(ThripsDam+1) + log(BlackPathDam+1) + WhiteFungLogis + (1 | Family) +  
    (1 | gh_bench/gh_col) + log(ThripsDam+1):log(BlackPathDam+1) + log(ThripsDam+1):WhiteFungLogis +  
    log(BlackPathDam+1):WhiteFungLogis+log(gluc_Conc)+Standardize(GM_Leaf_Area),data=MapleModel)
summary(fit7)
...
fit7<-lmer(GM_TotalLeaf_Area ~ Maple_TotalLeafArea_End + log(Fern + 1) +  
    log(ThripsDam+1) + log(BlackPathDam+1) + WhiteFungLogis + (1 | Family) +  
    (1 | gh_bench/gh_col) + log(ThripsDam+1):log(BlackPathDam+1) + log(ThripsDam+1):WhiteFungLogis +  
    log(BlackPathDam+1):WhiteFungLogis+log(flav_Conc)+Standardize(GM_Leaf_Area),data=MapleModel)
summary(fit7)
