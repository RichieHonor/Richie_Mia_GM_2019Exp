...
slopeCnt<-function(x){
  y=(1346.3+509.6)*x+4854.5+1210.0
  return(y)
}
...
ggplot()+
  ...
  geom_path(aes(x=xComp,y=slopeComp(xComp)),colour="#E69F00",size=1)+
    scale_colour_manual(values=c("#E69F00", "black"),labels=c("Maple-Garlic Mustard","Maple Alone"))+
...
fitTwFull<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+log(gluc_Conc)+MapleDamage+(1|Family)+(1|gh_bench),family = tweedie, data=MapleModel)

fitTwFull0.1<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+log(gluc_Conc)+MapleDamage+(1|gh_bench),family = tweedie, data=MapleModel)

fitTwFull0.2<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+log(gluc_Conc)+MapleDamage+(1|Family),family = tweedie, data=MapleModel)

fitTwFull0.3<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+log(gluc_Conc)+MapleDamage,family = tweedie, data=MapleModel)
...
fitTw1<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot*log(gluc_Conc)+MapleDamage,family = tweedie, data=MapleModel)
summary(fitTw1) #The interaction is not important, but glucosinolate concentration and maple damage are. 

fitTw2<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+log(gluc_Conc)+MapleDamage,family = tweedie, data=MapleModel[!is.na(MapleModel$gluc_Conc),])
summary(fitTw2)
...
fitTw3<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+MapleDamage+log(gluc_Conc)+log(ChlorA),family = tweedie, data=MapleModel)
summary(fitTw3)
...
fitV<-lm(Maple_TotalLeafArea_End~PC1Tot+MapleDamage+log(gluc_Conc)+log(ChlorA),data=MapleModel)
summary(fitV)
...
fitTw3<-glmmTMB(Maple_TotalLeafArea_End~PC1Tot+log(gluc_Conc)+MapleDamage,family = tweedie, data=MapleModel)
summary(fitTw3)
# 
# summary(fitTw3)$coef
...
```{r}
fit<-lm(log(gluc_Conc)~log(ChlorA)*treatment,data=dat)
fit2<-lm(log(gluc_Conc)~log(ChlorA)+treatment,data=dat)
anova(fit,fit2)
...
#Modelling random effects 
fit<-lmer(GM_TotalLeaf_Area~Maple_TotalLeafArea_End+Fern+ThripsDam*BlackPathDam*WhiteFungLogis+GM_Leaf_Area+(1|Family)+(1|gh_bench/gh_col),data=MapleModel)

fit2<-lmer(GM_TotalLeaf_Area~Maple_TotalLeafArea_End+Fern+ThripsDam*BlackPathDam*WhiteFungLogis+GM_Leaf_Area+(1|Family)+(1|gh_bench),data=MapleModel)

fit3<-lmer(GM_TotalLeaf_Area~Maple_TotalLeafArea_End+Fern+ThripsDam*BlackPathDam*WhiteFungLogis+GM_Leaf_Area+(1|Family),data=MapleModel)

fit4<-lmer(GM_TotalLeaf_Area~Maple_TotalLeafArea_End+Fern+ThripsDam*BlackPathDam*WhiteFungLogis+GM_Leaf_Area+(1|gh_bench),data=MapleModel)
...
#Is Bench important? 
anova(fit2,fit3) #Yes bench is very important

#Is family important? 
anova(fit2,fit4) #Yes family is very important

#Modeling fixed effects: 

fit<-lmer(GM_TotalLeaf_Area~Maple_TotalLeafArea_End+Fern+ThripsDam*BlackPathDam*WhiteFungLogis+GM_Leaf_Area+(1|Family)+(1|gh_bench),data=MapleModel)
...
fit6<-lmer(GM_TotalLeaf_Area ~ Maple_TotalLeafArea_End + Fern +  
    ThripsDam + BlackPathDam  +GM_Leaf_Area+(1 | Family)+(1|gh_bench)  ,data= MapleModel)
...
fit7<-lmer(GM_TotalLeaf_Area ~ Maple_TotalLeafArea_End   +  
    ...
    (1 | gh_bench) + GM_Leaf_Area+gluc_Conc,data=MapleModel)
...
fit_full<-glmmTMB(Fern~treatment+log(gluc_Conc)+log(ChlorA)+(1|Family)+(1|gh_bench),family=nbinom2,data=datFern)

fit_full_0.1<-glmmTMB(Fern~treatment+log(gluc_Conc)+log(ChlorA)+(1|Family),family=nbinom2,data=datFern)

fit_full_0.2<-glmmTMB(Fern~treatment+log(gluc_Conc)+log(ChlorA)+(1|gh_bench),family=nbinom2,data=datFern)

anova(fit_full,fit_full_0.1)#Bench is an extremenly important predictor. 
anova(fit_full,fit_full_0.2)#Family is not. 


fit_full<-glmmTMB(Fern~treatment+log(gluc_Conc)+log(ChlorA)+(1|gh_bench),family=nbinom2,data=datFern[!is.na(datFern$gluc_Conc),])
...
fit_1<-glmmTMB(Fern~treatment+log(ChlorA)+(1|gh_bench),family=nbinom2,data=datFern)
summary(fit_1)
