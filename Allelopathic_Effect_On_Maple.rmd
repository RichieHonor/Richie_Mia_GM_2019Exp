...
length(residuals(fitTreat))

fitTreatInt<-lm(Maple_TotalLeafArea_End~PC1Tot*treatment,data=Maple)
anova(fitTreat,fitTreatInt)#The interaction term is on the edge of significance. However, it is not technically significant. 
summary(fitTreatInt)


fitTreat<-lm(Standardize(Maple_TotalLeafArea_End)~PC1Tot+treatment,data=Maple)
summary(fitTreat)#This is highly significant effect of treatment.... 

confint(fitTreat)
fitTreat2<-lm(Standardize(Maple_TotalLeafArea_End)~PC1Tot,data=Maple)
anova(fitTreat,fitTreat2)
