...
#Adding in individual leaf size data. 
```{r}
...
main<-read.csv("Data_Moities/FertilizerExperiment/FertilizerSynthesis.csv",stringsAsFactors = F)

#Read in final leaf data files. 
dat<-read.csv("Data_Moities/FertilizerExperiment/GM_LeafPunchData_Fert.txt",stringsAsFactors = F)
...
dat$Tag<-gsub("§","|",dat$Tag)
dat$Tag<-gsub("_","?",dat$Tag)
...
dat$GM_Damage<-gsub("damage-","",dat$Damage)
#Creating Sample column which will be used to join collumns. 
dat$Sample<-paste(gsub(".*\\|","",dat$Tag), dat$GM_Leaf_Number,sep = "_")
dat$GM_Leaf_Area<-dat$GM_Leaf_Len*dat$GM_Leaf_Wid

#Removing unneccesairy columns
dat<-dat %>% select(-Sampletime,-Damage,-Tag,)
main<-main %>% select(-Maple_Area_mean_End)


Final<-left_join(main,dat,by="Sample")
