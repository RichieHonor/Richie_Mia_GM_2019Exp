...
#Making pca for glucosinolates and chlorophyll
...
# Run pca with gluc and chlor then do the G matrix again... all genetic variation of gluc exlained by chlorophyll? Do on whole data set and within each treatment. pc1, pc2 and total size. 
...
####GEnetic variation for PCA. 
datG<-dat2 %>% select(Tag,Family,PC1GC,PC2GC,GM_TotalLeaf_AreaS,gh_bench) %>%  pivot_longer(c(PC1GC,PC2GC,GM_TotalLeaf_AreaS))

glmmTMB(value ~ -1+name+(-1+name|Family),REML=T,data=datG)
