...
```{r}
fit8<-lmer(GM_TotalLeaf_Area~BlackPathDamW+gluc_Conc+(1|gh_bench),data=dat2)
summary(fit8)
#only black pathogen damage predicts performance in the alone treatment. 
```


#Checking if doing model selectionw with glucosinolate and flavonoids affects the outcome. What predicts garlic mustard performance in the alone treatment - gluc and flav Conc added.  (is there a cost to producing these compounds) 
```{r}
dat2<-dat2[dat2$treatment=="a",]
#Weighted pathogen damage data are no on too small of a scale. These will be standardized. 
...
#Checking what occurs when i add in chlorophyll A. interestingly, the slope for chlor A is negative, which does not make sense. Can only be explained by leaf area, as larger leaves will have less chlorophyll A and glucosinolates. I must control for this. 
fit12<-lmer(GM_TotalLeaf_Area~BlackPathDam+gluc_Conc+Standardize(GM_Leaf_Area)+(1|gh_bench),data=dat2)
summary(fit12) 
#After controlling for leaf area, there is no cost to glucosinoaltes as expected, on the contrary, they are positively correlated with performance. Unsurprising. 

#Cost to flavonoids?
fit12<-lmer(GM_TotalLeaf_Area~BlackPathDam+flav_Conc+Standardize(GM_Leaf_Area)+(1|gh_bench),data=dat2)
summary(fit12) 
#After controlling for leaf area, there is no cost to glucosinoaltes as expected, on the contrary, they are positively correlated with performance. Unsurprising. 
```

In summary only black pathogen damage predicts performance in the maple treatment, although there is almost a significant negative effect of glucosniolate concentration, suggesting a cost  of glucosinolates, this goes away after accounting for chlorophyll, suggesting any effect of glucoisnolates was simply correlative, and there is no cost to producing glucosinolates, likely because this trait is itself plastic with performance. If plants do not have enough nutrients, they will not invest in glucosinolates. Take home message. 
