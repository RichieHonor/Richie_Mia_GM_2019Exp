...
***Goal of Analysis***: To integrate the 11 different data sets I have surrounding *Alliaria petiolata* into a single data frame from which analysis and visualization can take place. 

***Data structure***: Each of the >600 plants are labeled with a unique number ID. This ID must be matched to the plant's genotype, and the location from which the genotype was obtained from around the world. There are data corresponding to each individual genotype in terms of: leaf chemistry ( up to 12 observations per plant), greenhouse data (e.g, greenhouse bench, initial size measurements), germination data (i.e. treatment with giberrelic acid, germination time), measurements of sugar maple competitors and control sugar maples, *Alliaria petiolata* growth rate data (5 different timed observations per individual), fern abundance in each pot and pathogen abundance on leaves, field location, survival data, and fecundity data. These data are all contained in separate files. 

***Approach***: Each file will be integrated in sequence to an ever growing "master file". For example, after the first two files are integrated, a master file (DataSynthesis.csv) is written with the integrated data. The next data file is then integrated into the master file (DataSynthesis.csv), which is then overwritten. This approach allows me to work on the integration of the data files throughout the thesis as new data files are created. 
...
* File for *fern abundance and moss presence* **fern_cover.txt**. 

* File for *Pathogen damage* **Pathogen_Damage_Manual.csv**

* File for *Glocosinolate,Flavonoid and Chlorophyll content* **All_UV_Data.txt**. in masters_thesis/UVSpec

* File for final leaf area (length and width of each leaf at end of first growing season) **Gm_Final_Leaf_data.txt** 

* File for *Field location and survival* **Data_Moities/Bolt_Field_Data_May132020.csv**. 

* File for *fecundity* **Data_Moities/Fecundity.csv**. 


**Notes**

* The final data frame and structure can be viewed at the end of the document

* This does not include data from the fertilizer experiment, which is separate from this integrate, and is properly integrated with the data pertaining to the fertilizer experiment in the document "FertilizerSynthesis.csv". 

* All data are now be located in "Data_Moieties"

# Integrating Greenhouse and UV Data. 
This incorporates treatment, ID and greenhouse location (Data_collection_GH_0.csv) with Glucosinolate, Flavonoid and chlorophyll data (All_UV_Data.txt).
...
#### Cleaning UV Data
This function averages the technical replicates
```{r}
#####
#Spec data prep--------------------------------------
...
Matching unique number ID to the genotype of the individual plant
```{r}
...
#### Removing duplicated data and checking for errors. 
...
There were two technical replicates for each leaf sample, two samples per leaf, and two leaves sampled per plant. Here they are averaged to the leaf level.
...
Some ID's have typos which are obvious and can be corrected manually.
```{r}
...
All_Wide$Sample[All_Wide$Sample=="381_1-Dat_Jan_21_2020_AM.txt"]<-"381_1"

All_Wide$Tag[All_Wide$Tag=="381_1"]<-"c|MSMID1-1-20|G381"

```



#### Merging clean greenhouse data with clean UV data (with technical replicate averaged)
```{r}
...
# Integrating for *fern abundance and moss presence* 
The file **fern_cover.csv** will be integrated with the data frame just constructed.
```{r, warning=F}
#Clear Environment. 
rm(list=ls())
library(dplyr)
...
# Integreating pathogen damage and final body mass of alliaria petiolata
The files below are integrated with the master synthesis file. 
...
# Integrating maple biomass data.
The final maple biomass data for the experimental maples, and the  initial and final maple control data will be integrated with the master file.Data are stored in: Data_Moities/Maple_data.txt and Data_Moities/Data_collection_GH.csv.
```{r, results = 'hide'}
rm(list=ls())
library(dplyr)
#------------------------------
...
#Select relevant columns. 
CntrZero<-dat %>% select(Tag,treatment,maple_leaf_length_0,maple_height_0,maple_leaves_0,gh_bench)
...
# Integrating Germination date and germination treatment. 
Found in Data_Moities/clean_germination_data_2.csv.
```{r, results='hide'}
rm(list=ls())
...
# Integrating Genotype location (longitude and lattitude).
Found in "Data_Moities/S0_population_info.csv".
```{r}
rm(list=ls())
...
DatJoin<-popinfo %>% filter(Pop_Code %in% dat$Family) %>% select(Pop_Code,Region,Latitude,Longitude,Altitude,Pop_Size,RosDens,AdultDens,TotalSizeS0=TotalSize,HerbivoryS0=Herb)
...
Found in "Data_Moities/Bolt_Field_Data_May132020.csv"
```{r, results ='hide', message = F}
rm(list=ls())
...
#First fix -> 160 is recorded in field data as both Jar and Smith. There should have been an i in front of smith. This will be fixed. .. 
...
anti_join(field,dat,by="ID") #Now there is just one ID in the field data without a match in the greenhouse data. 

#How many in the greenhouse did not get matched to those in the field? 
anti_join(dat,field,by="ID") %>% filter(treatment!="mcnt" & !grepl("i",ID))

#There are some that i know did not die in the greenhouse but are not in the field data. 
#and there are those that died in the greenhouse because they lack data taken at the end of the experiment such as total leaf area. 

#After removing those that were from north america, 13 individuals died in the greenhouse


field[!grepl("i",field$ID),]
...
# Integrating growth rate data
Growth rate data were organized in the "GrowthRate_Analysis.rmd". There I summarized the growth rate measurements into relative growth rate parameters which are to be used for data analysis. (integrating data in "Data_Moities/RGR_Dat.csv").
```{r,results ='hide', message = F}
rm(list=ls())
...
# Integrating fern data for the maple controls
Found in Data_Moities/fern_cover.csv". 
```{r}
#Clear Environment. 
rm(list=ls())
library(dplyr)
...
# Integrating fecundity data 
found in "Data_Moities/Fecundity.csv".
```{r,results = 'hide', message =F, fig.show='hide'}
