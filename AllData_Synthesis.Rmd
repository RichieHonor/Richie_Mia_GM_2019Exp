...
All_Wide$Sample[All_Wide$Sample=="a273_2_dupl-Dat_Dec_6_2019.txt"]<-"273d1_2"
...
All_Wide$Sample[All_Wide$Sample=="d273_JBCHY_1_dup-Dat_Dec_9_2019.txt"]<-"273d2_1"
...
#Adjusting for bad sampling days. 
Dec 6 and 8 runs used the protocol before we initialized the evaporation step, as a result, these samples will be adjusted for by bringing all samples down from the mean of their pool to the mean of the regular samples pool.  
```{r}
rm(list=ls())
dat<-read.csv("DataSynthesis.csv",stringsAsFactors = F)

#--------------------------------------------------------------
#Preparing data to be Adusted.
#--------------------------------------------------------------

#Isolating dec 6 and 8 trials to be adjusted
datOnlyFaults<-dat[dat$DataFile=="Dat_Dec_8_2019.txt"|dat$DataFile=="Dat_Dec_6_2019.txt",]

datOnlyFaultsPools<-datOnlyFaults[grepl("pool",datOnlyFaults$Sample),]

#Isolating pools from all data that are not from run dec 6 or 8.
datNoFaults<-dat[dat$DataFile!="Dat_Dec_8_2019.txt"&dat$DataFile!="Dat_Dec_6_2019.txt",]
datNoFaults<-datNoFaults[grepl("pool",datNoFaults$Sample),]

#Taking the mean of the pools without runs from dec 6 or 8. 
glucMean<-mean(datNoFaults$gluc_Conc)
flavMean<-mean(datNoFaults$flav_Conc)
ChlorAMean<-mean(datNoFaults$ChlorA)
ChlorBMean<-mean(datNoFaults$ChlorB)

#--------------------------------------------------------------
#Finding the different between the means of pools from runs on dec 6 and 8 from the grand mean of pools that do not contain those run means. 
#--------------------------------------------------------------

#Dec 6 differences.
glucDif6<-glucMean-datOnlyFaultsPools$gluc_Conc[datOnlyFaultsPools$Sample=="pool-Dat_Dec_6_2019.txt"]
flavDif6<-flavMean-datOnlyFaultsPools$flav_Conc[datOnlyFaultsPools$Sample=="pool-Dat_Dec_6_2019.txt"]
ChorADif6<-ChlorAMean-datOnlyFaultsPools$ChlorA[datOnlyFaultsPools$Sample=="pool-Dat_Dec_6_2019.txt"]
ChlorBDif6<-ChlorBMean-datOnlyFaultsPools$ChlorB[datOnlyFaultsPools$Sample=="pool-Dat_Dec_6_2019.txt"]

#Dec 8 Differences
glucDif8<-glucMean-datOnlyFaultsPools$gluc_Conc[datOnlyFaultsPools$Sample=="pool-Dat_Dec_8_2019.txt"]
flavDif8<-flavMean-datOnlyFaultsPools$flav_Conc[datOnlyFaultsPools$Sample=="pool-Dat_Dec_8_2019.txt"]
ChorADif8<-ChlorAMean-datOnlyFaultsPools$ChlorA[datOnlyFaultsPools$Sample=="pool-Dat_Dec_8_2019.txt"]
ChlorBDif8<-ChlorBMean-datOnlyFaultsPools$ChlorB[datOnlyFaultsPools$Sample=="pool-Dat_Dec_8_2019.txt"]

#--------------------------------------------------------------
#Now I simply add the difference between the true means without dec 6 and 8 and dec 6 and 8 means from the dec 6 and 8 values. 
#--------------------------------------------------------------

for(i in 1:length(datOnlyFaults$gluc_Conc)){
   if (datOnlyFaults$DataFile[i]=="Dat_Dec_6_2019.txt"){
    datOnlyFaults$gluc_Conc[i]<-datOnlyFaults$gluc_Conc[i]+glucDif6
    datOnlyFaults$flav_Conc[i]<-datOnlyFaults$flav_Conc[i]+flavDif6
    datOnlyFaults$ChlorA[i]<-datOnlyFaults$ChlorA[i]+ChorADif6
    datOnlyFaults$ChlorB[i]<-datOnlyFaults$ChlorB[i]+ChlorBDif6
  }
    else if (datOnlyFaults$DataFile[i]=="Dat_Dec_8_2019.txt"){
    datOnlyFaults$gluc_Conc[i]<-datOnlyFaults$gluc_Conc[i]+glucDif8
    datOnlyFaults$flav_Conc[i]<-datOnlyFaults$flav_Conc[i]+flavDif8
    datOnlyFaults$ChlorA[i]<-datOnlyFaults$ChlorA[i]+ChorADif8
    datOnlyFaults$ChlorB[i]<-datOnlyFaults$ChlorB[i]+ChlorBDif8
    }
  else{print("Missing Value...something went wrong.")
    print(datOnlyFaults$DataFile[i])}
}

#--------------------------------------------------------------
#Remerging fixed data frames. 
#--------------------------------------------------------------

#Recreating datNoFaults so that it contains the pool samples again.
datNoFaults<-dat[dat$DataFile!="Dat_Dec_8_2019.txt"&dat$DataFile!="Dat_Dec_6_2019.txt",]

#Merging
final<-rbind(datNoFaults,datOnlyFaults)
