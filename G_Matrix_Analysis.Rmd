...
dat2$FecundityS<-Standardize(dat2$GM_Fecundity)
dat2$gh_bench<-as.factor(dat2$gh_bench)
dat2$Bolt_Survival<-as.factor(dat2$Bolt_Survival)




```
#Gmatrix model of fecundity
```{r}
#First model will look at secondary compounds and fecundity.
datG<-dat2 %>% select(Tag,Family,gluc_ConcS,ChlorAS,flav_ConcS,GM_TotalLeaf_AreaS,gh_bench,treatment,FecundityS,Bolt_Survival) %>%  pivot_longer(c(GM_TotalLeaf_AreaS,FecundityS))

fit3<-glmmTMB(value ~ -1+name*treatment +(-1+name|Family),REML=T,data=datG) #Too small to fit. I think all genetic correlations of this sort will be too small to fit. 
