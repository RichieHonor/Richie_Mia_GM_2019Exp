...
datG<-dat2 %>% select(Tag,Family,gluc_ConcS,ChlorAS,flav_ConcS,GM_TotalLeaf_AreaS,gh_bench,treatment,RGR1S) %>%  pivot_longer(c(gluc_ConcS,GM_TotalLeaf_AreaS,ChlorAS))
hist(dat2$RGR1S)
...
fit<-lmer(GM_TotalLeaf_Area~treatment+BlackPathDam+WhiteFungLogis+ThripsDam+Fern+GM_Leaf_AreaS+(1|gh_bench), data=dat2)

fit1<-lmer(GM_TotalLeaf_Area~treatment+BlackPathDam+WhiteFungLogis+ThripsDam+GM_Leaf_AreaS+Fern+(1|gh_bench)+(1|Family), data=dat2)
anova(fit,fit1) #Family highly significant. 
summary(fit1) 
337263/(337263 +395515 + 5196413 )#Accounts for 5.7% of variation.

#GXE Interaction
fit2<-lmer(GM_TotalLeaf_Area~treatment+BlackPathDam+WhiteFungLogis+ThripsDam+GM_Leaf_AreaS+Fern+(1|gh_bench)+(1|Family:treatment), data=dat2)
anova(fit2,fit1) #Degrees of freedom is zero. I must fix this. 
summary(fit2) 
465806/(404221+5051469)
