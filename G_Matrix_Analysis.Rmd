...
#Gnetic variation for individual traits -- dont standardize those that are the y variable!
```{r}
#-----
####. CHlorophyll A

fit1<-glmmTMB(ChlorA ~ gh_bench+treatment+GM_Leaf_Area+(1|Family),REML=T,data=dat2)
fit<-glmmTMB(ChlorA ~ gh_bench+treatment+GM_Leaf_Area,REML=T,data=dat2)
anova(fit,fit1)
summary(fit1)
0.02171/(0.02171+0.86973)
#2.4 % of the variation in chlorophyll. # Chlorophyll genetic var not significant. 
...
fit<-glmmTMB(gluc_Conc ~ gh_bench+treatment+GM_Leaf_Area,REML=T,data=dat2)
anova(fit,fit1)
...
fit<-glmmTMB(RGR1 ~ gh_bench+treatment,REML=T,data=dat2)
anova(fit1,fit) #Family is not significant when greenouse bench is included? ?? Crazers
summary(fit1)

fit1<-glmmTMB(RGR1 ~ treatment+(1|Family),REML=T,data=dat2)
fit<-glmmTMB(RGR1 ~ treatment,REML=T,data=dat2)
anova(fit1,fit) #Family is not significant when greenouse bench is included? ?? Crazers
summary(fit1)
...
fit<-glmmTMB(GM_TotalLeaf_Area ~ gh_bench+treatment,REML=T,data=dat2)
anova(fit,fit1) #Family not significant for body size. This is different than what i found in the other analyiss. The difference is that significance is found when i include the other vairables that accoutn for genetic variation. Heritability goes up to around 5%.
summary(fit1)
