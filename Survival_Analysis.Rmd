...
#Best model
fit<- glmmTMB(Bolt_Survival ~ treatment + Standardize(PC1AllCGF) + Standardize(PC2All_CF) + Standardize(Fern) +  
    (1 | Family) + (1 | Row_Field) + treatment:Standardize(PC2All_CF),data=dat2, family="binomial")
summary(fit)

fitb<-update(fit,~.-Standardize(PC1AllCGF))
anova(fitb,fit)
0.4435-0.7435 #GM slope 
0.4435 -1.0268 #Maple slope 
```
...
anova(fit11,fit11b)
summary(fit11)
 -0.1375+0.1682 #GM slope
 -0.1375+0.6777 #Maple slope
...
fit1<-glmmTMB(Bolt_Survival~treatment+Standardize(GM_TotalLeaf_Area)+Standardize(RGR1)+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),],family="binomial")
summary(fit1)
#Is first year size significant? 
fit2<-glmmTMB(Bolt_Survival~treatment++Standardize(RGR1)+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),],family="binomial")
anova(fit1,fit2) #No. 


#IS growth rate? 
summary(fit1)
fit3<-glmmTMB(Bolt_Survival~treatment+Standardize(RGR1)+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$RGR1),],family="binomial")
fit4<-glmmTMB(Bolt_Survival~treatment++(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$RGR1),],family="binomial")
anova(fit3,fit4) #No. 

#Is treatment? 
fit3<-glmmTMB(Bolt_Survival~treatment+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$RGR1),],family="binomial")
fit4<-glmmTMB(Bolt_Survival~1+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$RGR1),],family="binomial")
anova(fit3,fit4) #No. 

fit3<-glmmTMB(Bolt_Survival~Standardize(GM_TotalLeaf_Area)+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),],family="binomial")
fit4<-glmmTMB(Bolt_Survival~1+(1|Family)+(1|Row_Field),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),],family="binomial")
anova(fit3,fit4) #No. 


#Fecundity
fit1<-glmmTMB(Standardize(GM_Fecundity)~Standardize(GM_TotalLeaf_Area)+Standardize(RGR1)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$PC1BoltSize),])
summary(fit1)
fit2<-glmmTMB(Standardize(GM_Fecundity)~Standardize(GM_TotalLeaf_Area)+Standardize(RGR1),data=dat2[!is.na(dat2$PC1BoltSize),])
anova(fit1,fit2) #Yes bolt size in second year is very important. 

#Is first year size?
fit1<-glmmTMB(Standardize(GM_Fecundity)~Standardize(GM_TotalLeaf_Area)+Standardize(RGR1)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),])
fit2<-glmmTMB(Standardize(GM_Fecundity)~+Standardize(RGR1)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),])
anova(fit1,fit2) #No

#IS relative growth rate?
fit1<-glmmTMB(Standardize(GM_Fecundity)~Standardize(RGR1)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$RGR1),])
fit2<-glmmTMB(Standardize(GM_Fecundity)~Standardize(PC1BoltSize),data=dat2[!is.na(dat2$RGR1),])
anova(fit1,fit2) #No
```
