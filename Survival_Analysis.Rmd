...
#Descriptive statistics of year 1 with competitiors removed. 
#```{r}
...
tapply(dat2$BlackPathDam,dat2$treatment,mean,na.rm=T)
tapply(dat2$BlackPathDam,dat2$treatment,ConfidenceInt)

tapply(dat2$ThripsDam,dat2$treatment,mean,na.rm=T)
tapply(dat2$ThripsDam,dat2$treatment,ConfidenceInt)

tapply(dat2$gluc_Conc,dat2$treatment,mean,na.rm=T)
tapply(dat2$gluc_Conc,dat2$treatment,ConfidenceInt)

tapply(dat2$flav_Conc,dat2$treatment,mean,na.rm=T)
tapply(dat2$flav_Conc,dat2$treatment,ConfidenceInt)

tapply(dat2$ChlorA,dat2$treatment,mean,na.rm=T)
tapply(dat2$ChlorA,dat2$treatment,ConfidenceInt)

tapply(dat2$GM_Leaf_Area,dat2$treatment,mean,na.rm=T)
tapply(dat2$GM_Leaf_Area,dat2$treatment,ConfidenceInt)


tapply(dat2$PC1BoltSize,dat2$treatment,mean,na.rm=T)

length(table(dat$Tag)[table(dat$Tag)>1])

length(table(dat$Tag)[table(dat$Tag)==1])
dat

length(table(dat$Tag[is.na(dat$Sample)&dat$treatment!="mcnt"])) #number of plants with no samples. 

dat2NoMaple<-dat2 %>% filter(treatment!="mcnt")

dat2NoMaple[is.na(dat2NoMaple$ChlorB),]
dat2NoMaple[!is.na(dat2NoMaple$ChlorB)&dat2NoMaple$NumberOfSamples==1,]
dat2NoMaple[!is.na(dat2NoMaple$ChlorB)&dat2NoMaple$NumberOfSamples>1,]
...
table(dat2$Col_Field,dat2$Row_Field)
...
1-exp(-0.82)/(exp(-0.82)+1)# % reduction in probability
...
#without efcc
fit11<-glmmTMB(Standardize(GM_Fecundity)~treatment*Standardize(PC1AllCGF)+Standardize(PC1BoltSize),data=dat2[!dat2$Family=="EFCC2",])
summary(fit11)
...
#Checking the results of AIC selection
```{r}

fit11<-glmmTMB(Standardize(GM_Fecundity)~treatment*Standardize(flav_Conc)+Standardize(PC1BoltSize),data=dat2)
AIC(fit11)
fit11<-glmmTMB(Standardize(GM_Fecundity)~treatment*Standardize(gluc_Conc)+Standardize(PC1BoltSize),data=dat2)
AIC(fit11)
fit11<-glmmTMB(Standardize(GM_Fecundity)~treatment*Standardize(ChlorA)+Standardize(PC1BoltSize),data=dat2)
AIC(fit11)
...
mapleSlope<-function(x){
  y=(-0.38505 +0.73142)*x-0.35393 -0.17813
  return(y)
}

mustardSlope<-function(x){
  #y=(-0.38505 +0.37816)*x-0.35393-0.65820
  y=0*x-0.35393-0.65820
  return(y)
}
...
datVis$GM_TotalLeaf_AreaS<-Standardize(datVis$GM_TotalLeaf_Area)
#tiff("Selection_Figures/LeafQualityOnFecundity.tiff", units="in", width=8, height=6, res=300)
FecTiff<-ggplot(datVis)+
  geom_point(aes(y=Standardize(GM_Fecundity),x=PC1AllCGFS,colour=treatment,shape=treatment),size=2)+
...
tiff("Selection_Figures/PC1SelectionMultiNoLegend_NullHyp.tiff", units="in", width=14, height=7, res=300)
top
dev.off()
...
fit_full1<-glmmTMB(Bolt_Survival~treatment+Standardize(Fern)+Standardize(PC1BoltSize)+(1|Family)+(1|Row_Field),data=datFern2,family="binomial")

fit_full2<-glmmTMB(Bolt_Survival~treatment+Standardize(PC1BoltSize)+(1|Family)++(1|Row_Field),data=datFern2,family="binomial")
anova(fit_full1,fit_full2) #Fern just significant
...
#Effect on fecundity update with treatment*PCA interactions - Retaining family in analysis.  
```{r}
...
#IS family  significant?
anova(fit5,fit2) #No.. fairly close however. 0.057.. GOING TO KEEP IT

###Modeling fixed effects 

#Is Total leaf area (year 1) significant?
fit0.1<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+Standardize(Fern)+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(RGR1)+Standardize(GM_TotalLeaf_Area)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),])
fit1<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+Standardize(Fern)+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(RGR1)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$GM_TotalLeaf_Area),])
anova(fit1,fit0.1)# no

#Is RGR significant?
fit0.1<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+Standardize(Fern)+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(RGR1)+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$RGR1),])
fit1<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+Standardize(Fern)+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$RGR1),])
anova(fit1,fit0.1)# no

#Is Bolt Size?
fit0.1<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+Standardize(Fern)+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(PC1BoltSize),data=dat2[!is.na(dat2$PC1BoltSize),])
fit1<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+Standardize(Fern)+BlackPathDam*WhiteFungLogis*ThripsDam,data=dat2[!is.na(dat2$PC1BoltSize),])
anova(fit1,fit0.1)# Yes.... extremely.
...
fit3<-update(fit2,~.-BlackPathDam:WhiteFungLogis)
anova(fit3,fit2) #There is a significant two way interaction. Black Path Dam and white fung logis. 
...
#IS the interaction between treatment and PC1AllCGF significant?
fit9<-update(fit8,~.-treatment:PC1AllCGF)
anova(fit9,fit8) #Yes, the interaction between PC1AllCGF and treatment is significant. p=0.005.. more significant than before. 
...
#IS thrips dam significant?
fit10<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+Standardize(Fern)+BlackPathDam+WhiteFungLogis+ThripsDam,data=dat2[!is.na(dat2$ThripsDam),])
fit11<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+Standardize(Fern)+BlackPathDam+WhiteFungLogis,data=dat2[!is.na(dat2$ThripsDam),])
anova(fit10,fit11) #No.

#Is The interaction significant?
fit11.b<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+Standardize(Fern)+Standardize(BlackPathDam)*WhiteFungLogis,data=dat2[!is.na(dat2$ThripsDam),])
anova(fit11,fit11.b) #Yes...



#IS Fern significant?
fit10<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+Standardize(Fern)+BlackPathDam*WhiteFungLogis,data=dat2[!is.na(dat2$Fern),])
fit11<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+BlackPathDam*WhiteFungLogis,data=dat2[!is.na(dat2$Fern),])
anova(fit10,fit11) #No.

#IS interaction significant?
fit10<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+Standardize(Fern)+BlackPathDam*WhiteFungLogis,data=dat2[!is.na(dat2$Fern),])
fit11<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*PC1AllCGF+Standardize(Fern)+BlackPathDam+WhiteFungLogis,data=dat2[!is.na(dat2$Fern),])
anova(fit10,fit11) #Yes

#The simplest best model is

fit11<-glmmTMB(Standardize(GM_Fecundity)~(1|Family)+treatment*Standardize(PC1AllCGF)+Standardize(PC1BoltSize)+Standardize(BlackPathDam)+WhiteFungLogis,data=dat2)
summary(fit11)
