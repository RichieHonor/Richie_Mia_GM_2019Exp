...
passive<-ggplot(Vis,aes(x = treatment, 
...
active<-ggplot(Vis,aes(x = treatment, 
                    y = estimate, colour = treatment))+
  geom_point(size = 5)+
  geom_errorbar(ymin = Vis$lower,
                ymax = Vis$upper, width = 0.1)+
...
#Visualization of treatment means glucosinolate
```{r}
#Visualizing passive plasticity
a<--2.21675805
...
tiff("Plasticity_Figures/FlavonoidPassive_Plasticity.tiff", units="in", width=10, height=6, res=300)
passive
dev.off
```
...
plot_grid(gluc,flav,ncol=2,nrow=1,labels=c("a","b"))
dev.off()
