...
summary(lmer(GM_TotalLeaf_Area ~ treatment*gluc_Conc  + (1 | gh_bench) ,data=dat2))
...
ggplot(dat2)+
  ...
  ylab(bquote(bold("Shoot Area "(mm^2))))+xlab(bquote(bold("[Total Glucosinolate] " (mg/ml))))+
...
ggplot(plot)+
  geom_col(aes(x=treatment,y=PercWhitFung))
...
fit.G<-glmer.nb(WhiteFungDam~treatment+gluc_Conc+(1|Family),data=datWFnonzero)
...
ggplot(dat[!is.na(dat$flav_Conc)&!is.na(dat$flav_Conc),])+
  ...
  ylab("Thrips Damage")+
xlab(bquote(bold("[Total Flavonol] " (mg/ml))))
#dev.off()
