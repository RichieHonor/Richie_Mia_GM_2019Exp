...
#Bad Model Thrips for non-significant effect.
fit6<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*Standardize(PC1AllCGF)+Standardize(PC2All_CF)+Standardize(PC3All_CG)+Standardize(BlackPathDam)+WhiteFungLogis+Standardize(GM_Leaf_Area)+Standardize(RGR1)+(1|Family)+(1|gh_bench), data=dat2[!is.na(dat2$BlackPathDam),])
summary(fit6)
summary(fit)
