...
fitfull<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(GM_Leaf_Area)+(1|Family)+(1|gh_bench), data=dat2)

fitfull1<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(GM_Leaf_Area)+(1|gh_bench), data=dat2)

fitfull2<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*PC1AllCGF+treatment*PC2All_CF+treatment*PC3All_CG+BlackPathDam*WhiteFungLogis*ThripsDam+Standardize(GM_Leaf_Area)+(1|Family), data=dat2)
...
#IS the interaction between treatment and PC1AllCGF significant?
fit13<-update(fit12,~.-treatment:PC1AllCGF)
anova(fit13,fit12) #Yes it is significant after thrips was removed. ... 0.0487
...
#With Thrips for non-significant effect.
fit6<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment*Standardize(PC1AllCGF)+Standardize(BlackPathDam)+WhiteFungLogis+Standardize(GM_Leaf_Area)+Standardize(ThripsDam)+(1|Family)+(1|gh_bench), data=dat2[!is.na(dat2$BlackPathDam),])
summary(fit6)
summary(fit)
...
mapleSlope<-function(x){
  y=(0.07136+0.21534)*x+0.34158 -0.23135
  return(y)
}

mustardSlope<-function(x){
  y=(0.07136+0.02952)*x+0.34158-0.61785#Non significant slope
  return(y)
}
...
tiff("Selection_Figures/LeafQuality.tiff", units="in", width=8, height=6, res=300)
ggplot(dat2)+
  geom_point(aes(y=GM_TotalLeaf_AreaS,x=PC1AllCGFS,colour=treatment),size=2)+
  ...
  geom_segment(x=minG,xend=maxG,y=mustardSlope(minG),yend=mustardSlope(maxG),colour="#56B4E9",size=1.5)+theme_simple_vert()+
  ylab(bquote(bold("Total Leaf Area (σ)")))+xlab("Leaf Quality (σ)")+
...
#Treatment effect 
fit_full1<-glmmTMB(Standardize(GM_TotalLeaf_Area)~treatment+Standardize(BlackPathDam)+WhiteFungLogis+Fern+(1|gh_bench),data=datFern2)
fit_full2<-glmmTMB(Standardize(GM_TotalLeaf_Area)~Standardize(BlackPathDam)+WhiteFungLogis+Fern+(1|gh_bench),data=datFern2)
anova(fit_full1,fit_full2) #Treatment highly significant
